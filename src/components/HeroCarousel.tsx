...
export const HeroCarouselComponent = ({
  ...
  return (
    <section
      ...
      className="w-full relative rounded-[20px] sm:rounded-[36px] md:rounded-[42px] select-none group/carousel"
    >
      {/* Main Carousel Frame (Two-Zone Split on Desktop, exact match to screenshot) */}
      <div className="relative z-10 w-full overflow-hidden rounded-[20px] sm:rounded-[36px] md:rounded-[42px] glass-panel border border-white/80 dark:border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.15)] dark:shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
          ...
          >
            ...
            {currentSlide.type === "adsense" ? (
              ...
              <div className="w-full flex flex-col-reverse md:flex-row p-4 sm:p-6 md:p-8 lg:p-10 gap-6 md:gap-8 items-center justify-between">
                ...
                {/* LEFT SIDE: Information Deck */}
                <div className="w-full md:w-1/2 flex flex-col justify-between z-10">
                  <div>
                    {/* Top Badges */}
                    <div className="flex items-center gap-2 mb-3 sm:mb-4 flex-wrap">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/40 dark:bg-white/10 backdrop-blur-md border border-white/15 text-[10px] sm:text-xs font-heading font-black tracking-wider text-amber-400 dark:text-orange-400 uppercase shadow-xs">
                    ...
                    {/* Title */}
                    <h1 className="text-xl min-[390px]:text-2xl sm:text-3xl md:text-4xl lg:text-[40px] font-black font-heading text-gray-900 dark:text-white tracking-tight leading-tight mb-2 sm:mb-3 line-clamp-2 break-safe">
                      {currentSlide.title}
                    </h1>

                    {/* Description */}
                    {currentSlide.description && (
                      <p className="text-gray-600 dark:text-gray-300 text-xs sm:text-sm md:text-base leading-relaxed line-clamp-2 mb-4 sm:mb-6 break-safe">
                        {currentSlide.description}
                      </p>
                    )}

                    {/* Information Stack Card */}
                    <div className="rounded-[18px] sm:rounded-2xl p-3.5 sm:p-5 bg-black/[0.03] dark:bg-black/40 border border-black/5 dark:border-white/10 space-y-2.5 sm:space-y-3 mb-5 sm:mb-8">
                      {/* Date & Time */}
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-orange-500/15 text-primary border border-orange-500/25 flex items-center justify-center shrink-0">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-300 truncate">
                          By <strong className="text-gray-900 dark:text-white font-extrabold">{currentSlide.organizer || "LPU Club"}</strong>
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {/* RIGHT SIDE: Large Cinematic Image Stage */}
                <div className="w-full md:w-1/2 flex items-center justify-center">
                  <div
                    onClick={handleAction}
                    className="relative w-full h-[220px] min-[390px]:h-[250px] sm:h-[340px] md:h-[460px] rounded-[20px] sm:rounded-[30px] md:rounded-[34px] overflow-hidden shadow-2xl bg-slate-900 border border-white/80 dark:border-white/10 group/img cursor-pointer"
                  >
                    <img
        ...
        {slides.length > 1 && (
          <>
            <button
              ...
              className="hidden sm:flex absolute left-3 top-1/2 -translate-y-1/2 z-30 w-10 h-10 md:w-11 md:h-11 rounded-full glass-pill items-center justify-center cursor-pointer shadow-xl border border-white/95 dark:border-white/20 bg-white/90 dark:bg-black/85 backdrop-blur-2xl transition-transform hover:scale-110 active:scale-95"
            ...
            <button
              ...
              className="hidden sm:flex absolute right-3 top-1/2 -translate-y-1/2 z-30 w-10 h-10 md:w-11 md:h-11 rounded-full glass-pill items-center justify-center cursor-pointer shadow-xl border border-white/95 dark:border-white/20 bg-white/90 dark:bg-black/85 backdrop-blur-2xl transition-transform hover:scale-110 active:scale-95"
        ...
        {slides.length > 1 && (
          <div className="absolute bottom-3 sm:bottom-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/40 dark:bg-black/60 backdrop-blur-md border border-white/15">
            {slides.map((_, idx) => (
              <button
                key={idx}
                type="button"
