-- 20260818000001_high_quality_search_events.sql
-- High-Quality, Low-Database-Load Global Search for LPU Events

-- Add trigram indexes on searchable fields if not already present
...
CREATE INDEX IF NOT EXISTS subcategories_name_trgm_idx ON public.subcategories USING gin (name gin_trgm_ops);

-- Replace search_events function with weighted relevance model & filter support
  ...
  WITH scored AS (
    SELECT 
      ...
      (
        ...
        END)
        +
        -- Priority 4: Club / Organizer (50 / 30)
        (CASE 
...
GRANT EXECUTE ON FUNCTION public.search_events(text, integer, integer, uuid, uuid, public.event_pricing_type, text, date, boolean) TO anon, authenticated;
