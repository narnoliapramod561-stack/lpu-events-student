-- ==============================================================================
-- LPU Events — Canonical Database Seed Script
-- ==============================================================================

-- 0. Clean up existing transactional & platform data in reverse dependency order
...
-- 1. Ensure Super Admin user in auth.users
...
-- 2. Seed Organizer Users in auth.users
insert into auth.users (
  id, instance_id, aud, role, email, encrypted_password, email_confirmed_at,
  created_at, updated_at, raw_app_meta_data, raw_user_meta_data, is_super_admin,
  confirmation_token, recovery_token, email_change_token_new, email_change,
...
values
  (
    ...
    'authenticated', 'authenticated', 'organizer@lpu.in',
    '$2a$10$w0998c08w898w098w898w098w898w098w898w098w898w098w898w',
    now(), now(), now(), '{"provider":"email","providers":["email"]}',
    '{"email":"organizer@lpu.in","display_name":"Technical Lead"}',
    false, '', '', '', '', '', '', '', ''
  ),
  (
    'a0000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000000',
    'authenticated', 'authenticated', 'cultural.lead@lpu.in',
    '$2a$10$w0998c08w898w098w898w098w898w098w898w098w898w098w898w',
    now(), now(), now(), '{"provider":"email","providers":["email"]}',
    '{"email":"cultural.lead@lpu.in","display_name":"Cultural Affairs Lead"}',
    false, '', '', '', '', '', '', '', ''
  )
on conflict (id) do update set
  email_confirmed_at = now(),
...
select
  '11111111-1111-1111-1111-111111111111', id, 'ORGANIZER'::public.organization_member_role
from public.admin_users where email = 'organizer@lpu.in'
on conflict do nothing;

insert into public.organization_members (organization_id, admin_user_id, role)
select
  '22222222-2222-2222-2222-222222222222', id, 'ORGANIZER'::public.organization_member_role
from public.admin_users where email = 'cultural.lead@lpu.in'
on conflict do nothing;

-- 5. Seed Organizer Access Requests
insert into public.organizer_access_requests (admin_user_id, organization_name, remarks, status)
select
  id, 'IEEE LPU Student Branch',
  'Requesting organizer workspace access for IEEE international technical symposiums',
...
insert into public.organizer_access_requests (admin_user_id, organization_name, remarks, status)
select
  id, 'Fine Arts & Cultural Society',
  'Requesting access to host regional drama and painting competitions',
...
on conflict (id) do update set name = excluded.name, key = excluded.key, sort_order = excluded.sort_order, is_active = true;
